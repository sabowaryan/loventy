/*
  # Fix template_id type in invitations table

  1. Problem
    - The template_id column in invitations table is currently of type text
    - The id column in invitation_templates table is of type uuid
    - This causes inconsistency and makes it difficult to properly reference templates

  2. Solution
    - Create a temporary column to store the converted UUIDs
    - Update the temporary column with converted values
    - Drop the old column and rename the new one
    - Add a foreign key constraint to properly reference invitation_templates.id

  3. Views
    - Update affected views to use the new column type
*/
...
-- Drop views that depend on the invitations table to avoid dependency issues
...
-- Recreate user_template_stats view
