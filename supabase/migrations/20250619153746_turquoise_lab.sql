...
-- Recreate template_details view with SECURITY INVOKER explicitly set
CREATE VIEW template_details WITH (security_invoker = true) AS
SELECT 
  t.id,
  t.name,
  t.slug,
...
-- Recreate invitation_details view with SECURITY INVOKER explicitly set
CREATE VIEW invitation_details WITH (security_invoker = true) AS
SELECT 
  i.id,
  i.user_id,
  i.title,
...
-- Recreate guest_details view with SECURITY INVOKER explicitly set
CREATE VIEW guest_details WITH (security_invoker = true) AS
SELECT 
  g.id,
  g.invitation_id,
...
-- Recreate user_template_stats view with SECURITY INVOKER explicitly set
CREATE VIEW user_template_stats WITH (security_invoker = true) AS
SELECT 
  u.id as user_id,
  u.email,
  t.id as template_id,
...
-- Recreate media_details view with SECURITY INVOKER explicitly set
CREATE VIEW media_details WITH (security_invoker = true) AS
SELECT 
  im.id,
  im.invitation_id,
  i.title as invitation_title,
...
-- Recreate user_usage_stats view with SECURITY INVOKER explicitly set
CREATE VIEW user_usage_stats WITH (security_invoker = true) AS
SELECT 
  u.id as user_id,
  u.email,
