...
const EditorSidebar: React.FC<EditorSidebarProps> = ({
  ...
  const sectionsByCategory = {
    ...
    design: [
      ...
      { id: 'section-design', name: 'Design par section', icon: Layout, description: 'Contrôle détaillé des sections' },
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden h-full flex">
      ...
      <nav className="flex flex-col flex-shrink-0 w-48 border-r border-gray-200">
        {mainCategories.map((category) => (
          <button
            key={category.id}
            onClick={() => {
              setActiveTab(category.id);
              // Automatically set the first section of the new category as active
